...
const Balance = () => {
  ...
  const BuildBalanceArray = () => {
    ...
    db_products.map((prod: IProdProps) => {
      const entries = db_stock_entries
        .filter((item: IItensProps) => item.product_id === prod.id)
        .map((entry: IItensProps) => Number(entry.amount))
  ...
  const handleFilterProducts = () => {
    ...
    const newArray = cmbProducts.filter(
      (item: IItensProps) => item.product_id === productFiltered
    );
        ...
        <Box w="100%">
          <SimpleGrid minChildWidth={240} h="fit-content" spacing="6">
            ...
            >
              ...
              {cmbProducts &&
                ...
                cmbProducts.map((item: IItensProps, i) => (
          ...
          <Box overflowY="auto" height="80vh">
            <Table mt="6">
              ...
              <Tbody>
                {listProducts.map((item: IItensProps, i) => (
                  <Tr key={i}>
